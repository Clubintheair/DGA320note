...
* 当出现FCU 1+2时，AP、A-THR、FD都会断开，因此飞行员按照"4.11 金科玉律"[^1]第一条，应该在第一时间操纵好飞机的状态：

  * 使用侧杆接管按钮接管飞机，从而止铃
  * 尽快将推力手柄设置到当前的推力实际位置，并且按压自动推力断开按钮
  * 通话报告ATC，如在RVSM区域，则应该脱离当前区域，详细程序请参考："RVSM紧急脱离程序"[^2]
* 根据"3.3 非正常和紧急程序"[^3]进行处置，完成"3.3.1 天龙八步"[^4]
* 使用"原始数据导航"[^17]技术进行进近
...
[^17]: # 原始数据导航
    ...
    * 两部FMGC失效，丢失FM功能，需要使用原始数据导航
    ...
    无论发生上述任何情况，使用原始数据导航飞行在本质上并不困难，机组都应该按照"4.11 金科玉律"[^1]的原则执行`飞行、导航、通讯`任务排序，进而更好的将飞机控制在安全包线范围之内
    ...
    在进行原始数据导航的训练时，经常出现由于刚起飞就是去导航精度而需要备降或返场的情况，在这种情况下，尤其是双FMGC故障时，很容易出现由于飞机没有显示全重，或者查询全重后未引起警惕而在无意间超重着陆的情况，为了避免这样的情况，机组应该通过"7. 限制"[^27]对自己当前的飞机重量进行查询，如果发现飞机超重，则需要在空中加入等待，而标准等待的加入往往也是在进行原始数据导航时所附加的考核项目
    ...
    * 将ND显示模式选择至F-PLAN模式（除非"FCU 1+2故障"[^29]，其他情况都可以使用），因为F-PLAN模式是所有模式中以正北为基准显示的模式：
...
[^18]: # 地图漂移
    ...
    * 虽然很可能在第一时间不会出现ECAM动作，但是依旧按照"3.3 非正常和紧急程序"[^3]处置流程进行处置，PF通知ATC当前飞机状态，并且申请雷达引导
    ...
    * 为什么同样是使用原始数据导航，地图漂移就需要关闭GPWS系统的TERR功能，而双MCDU失效和双FMGC失效就不需要？
      ...
      * GPWS的五个基本提醒功能包括：
        ...
        * 下降低于下滑道过大
        ...
        这几个功能都和气压高度有关，和当前数据库是否生效没有直接关系，但是当出现地图漂移时，系统提供了错误的地形数据，虽然从系统逻辑上来讲，某些新型的EPGWS系统在探测到导航精度低时，会自动抑制EGPWS的TAD（地形意识和显示）、TCF（地形间隔平台）、RFCF（跑道间隔区域底线）功能，只保留作为GPWS的五个基本功能， 但是由于每一架飞机使用的GPWS型号不同，如果没有进行抑制的话，在进近过程中就很容易激活错误的可控撞地提醒，一旦进行了提醒，无论是否是假警告，机组都应该按照记忆项目的原则执行"GPWS及EGPWS"[^19]程序，导致没有必要的复飞，因此，最好的办法是机组不需要投入过多的精力去进行当前机型安装GWPS类别的查询，而直接关闭GPWS的TERR功能，达到避免虚假警告的目的
...
[^19]: # GPWS及EGPWS
    ...
    * 调整俯仰和推力，使警报停止。
    * 当高度低于500AGL时，考虑复飞，参考"中止进近、复飞、中断着陆"[^20]
    ...
    * 复飞，参考"复飞程序："[^23]
    ...
    在VMC条件下，并且确认飞机没有可控撞地风险且负荷稳定进近的定义（参考："稳定进近"[^24]）；或在执行运营人设计的特殊程序时；或在有意低于下滑道时，飞行机组可以：
...
[^20]: # 中止进近、复飞、中断着陆
    ...
    在从进近开始的所有中止继续进近的行为都可以认为是中止进近，包括复飞。中止进近可以视实际情况使用"中止进近程序："[^21]或"复飞程序："[^23]。
    ...
    * 宣布“取消进近”
    * 取消已选择的进近模式，参考"FCU上各键位的作用和断开方式："[^22]
    * 管理飞机的航径
    * 如需要，改航
...
[^24]: # 稳定进近
    ...
    * 飞机过跑道时，应该：
    ...
    注意：在整个进近过程中如果出现任何不能满足以上条件的情况，机组都应该选择复飞，复飞内容请参阅"中止进近、复飞、中断着陆"[^20]
...
[^25]: # 双MCDU失效
    ...
    * 虽然没有ECAM程序，但是也应该按照"3.3 非正常和紧急程序"[^3]顺序进行处置
...
[^26]: # 双FMGC失效
    ...
    8. 使用舱单上ZFW+当前FOB的方式重新计算当前飞机的重量，得到当前飞机全重之后，根据重心查询着陆距离及进近速度，如果当前飞机重量超过最大着陆重量（参考"7. 限制"[^27]），应当联系ATC使用原始数据导航加入等待耗油
