...
* ECAM警告
* 琥珀色<u>ENG 1(2)</u> **FADEC FAULT**
* E/WD上所以监控数据失去，以琥珀色X显示

# **FADEC 故障的具体程序**

* 当出现FADEC 故障警告，飞行员应该立刻按照"3.3 非正常和紧急程序"[^1]原则接管飞机，并且划分工作区域
* 执行ECAM动作
* 查询FCOM中的相关内容
* 根据引气、液压、电气系统的工作状态来判断发动机的实际工作状态
  ...
  * 如发动机工作不正常：
    ...
    * 受影响的发动机主电门关
    * 如在空中，按照空中单发程序处置(参考："单发"[^16])
    * 尽快落地

# **容易出现的问题**

* 误判为单发(跳过ECAM上的白字条件项)
* 不根据引气、液压、电气系统的工作状态来对发动机的工作状态进行判断，盲目关车
* 当发动机正常工作时，决策继续执行航班
...
* FADEC故障的误判：
  ...
  * 当出现FADEC故障时，ECAM警戒响起，E/WD上的所以指示消失，由于AP在接通状态，机组无法直接感受到操纵上的变化，在初步判断为发动机系统故障后，在执行ECAM时容易紧张或急躁，因此容易跳过ECAM上的白色字（条件项），造成当前飞机状态为单发的误判，直接按照单发程序处置
* FADEC故障后，为什么不能按照单发处置？

  * 当FADEC故障后，虽然系统无法直接对发动机的状态进行监控，但是发动机本身的工作状态正常，虽然从性能上来讲直接以单发处置的问题不大，但是从法规上来讲，空中关车属于严重事故征候，因此需要尽量避免
* 既然发动机本身的工作状态正常，那在这样的情况下为什么还需要尽快落地？

  * 即便当前发动机工作正常，不代表后续的发动机可以一直保持正常运转，而且虽然可以通过相关的三个系统进行侧面的监控，但是毕竟不够直观，且会占用机组大量精力。按照空客的系统设计原则(参考"3. A320程序设计"[^24])，任何一套系统失效后都需要有备份的系统，否则应该视为飞机当前不适航。因此如果当前飞机失去了对发动机的监控方式，我们可视为继续带故障飞行不安全，因此建议尽快落地排故
* 判明当前飞机状态为FADEC故障，发动机工作正常后，要不要做计算机复位？

  * 目前该问题有两个不同的观点，请结合自己公司对于相关科目的要求自行决定，此处只列出观点：

    * 需要复位：

      FADEC 本身具有两个通道，FADEC故障可能是由于两个通道同时失效造成的，尝试复位如果成功，可能使FADEC功能恢复到晋为降级而非失效飞机依旧可以继续执行航班
    * 不需要复位：

      * 空客本身在进行程序设计的时候就已经考虑到FADEC故障的情况，FCOM上针对该故障的描述也没有提到复位，其原因在于，如果A、B通道同时失效，FADEC可恢复的几率不大，即FADEC本身已经故障，和其通道无关。执行复位会占用机组大量精力，且，此时发动机大概率在正常工作，即便是没有正常工作，在绝大部分情况下，单发的性能也能够满足备降或返场的要求，因此没有必要进行复位

# FADEC 故障视频演示

<iframe src="https://player.bilibili.com/player.html?aid=807791468&amp;bvid=BV1W34y1z7J2&amp;cid=475451743&amp;page=1" data-src="//player.bilibili.com/player.html?aid=807791468&amp;bvid=BV1W34y1z7J2&amp;cid=475451743&amp;page=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"></iframe>
...
[^17]: # V1前单发
    ...
    * ECAM警告响起，**警告灯亮**，并伴有连续警告音（发动机火警）
    ...
    | **单发无损** | **单发有损** | **单发火警**                                  |
    | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
    | 参考"单发时发动机可能的状态："[^18]                                           | 参考"单发时发动机可能的状态："[^18]<br />                                         | 火警灯亮，有连续警告音，ECAM出现**红色火警**提示，判断为火警<br /> |
    | ECAM动作做到单发关断，报出发动机保护完成<br />                                  | ECAM动作做到第一个灭火瓶释放，报出发动机保护完成<br />                          | ECAM执行到第二个灭火瓶释放<br />                                                                                  |
    | PA广播乘务组留在原地无需撤离，CM2联系ATC申请飞机滑回检查<br />                  | PA广播乘务组留在原地无需撤离，CM2联系ATC申飞机滑回检查<br />                    | 判断是否需要紧急撤离，如需要，执行"紧急撤离"[^19]程序，如不需要，通知乘务组解除警戒<br />                           |
    | <br />                                                                          | <br />                                                                          | 如无法自行滑回，CM2联系ATC申请拖车<br />                                                                          |

    # **V1前单发应该重点关注的问题：**

    * 发动机一旦出现单发或火警，可能出现的是发动机无损但非正常关车，发动机有损非正常关车，发动机带火警且关车，发动机带火警且还在工作。正确的判断当前发动机的实际状态可以让机组避免无谓的释放灭火瓶动作，减少公司的损失，而一味的不释放灭火瓶，在飞机发动机有损坏或带火警的情况下，则可能造成严重的安全隐患，进一步扩大故障带来的伤害和后果，因此当前发动机实际状态的判断尤为重要，详细信息请参考"单发时发动机可能的状态："[^18]

    # **低速单发中断的要点：**

    * 低速中断的危险性，参考"单发时方向的控制："[^20]
    * 处置要点：
...
[^21]: # V1后单发
    ...
    * ECAM警告响起，**警告灯亮**，并伴有连续警告音（发动机火警）
    ...
    | 如起动成功，则尽快落地<br />                                                    | 尽快落地<br />                                                                  | 做好"紧急撤离"[^19]预案<br />                                                      |
